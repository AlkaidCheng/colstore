...
ds.n_rows          # row count  (use this, NOT len()/count())
...
for batch in fr.iter_batches("256 MiB"):     # bounded-memory streaming over a large edit
    ...
```
...
`store(..., batch_size=...)`, `create/recreate/update`, and `compact` take `show_progress=False`
to silence the progress bar. Writes commit atomically via a counters block — a reader opening
mid-write sees only the last committed state.
...
`.append(source)` and `.needs_compaction`.
...
```python
# export a reader / dataset / view:
ds.arrow()                          # zero-copy pyarrow Table  (== ds.to("arrow"))
ds.to_parquet("out.parquet")        # also .to_feather/.to_json/.to_npz/.to_hdf
ds.saveas("out.parquet")            # dispatch by extension (or format=...)
colstore.to_root(ds, "out.root")    # ROOT (needs uproot/ROOT)

# import a foreign file into a new .cstore and open it:
...
- **`compact` once, read zero-copy many.** A single-record store gives `copy=False` views; a
  multi-record one copies. Compact files you read repeatedly.
...
- **Row count is `n_rows`** (a property), and `shape`/`columns`/`dtypes` are properties — there is
  no `len()`/`count()` on a reader (frames have `count()` for cutflow survivors).
...
- `ColStoreReader(path, *, madvise=, mlock=, backend=, max_workers=)` — indexing → `ColumnView`/`TableView`; `.array/.dict/.recarray/.frame(copy=True)`, `.query/.where/.select/.drop`, `.head/.tail`, `.edit()`, `.arrow/.to/.saveas/.to_*`; props `.shape/.n_rows/.columns/.dtypes`.
- `ColumnView`, `TableView` — lazy views; `.evaluate()` fixes the row mask; same materializers.
...
**Datasets & shards**
- `ColStoreDataset(sources)` — many files as one table; reader API plus `.append`, `.needs_compaction`.
...
**Editing**
- `ColStoreFrame` (via `reader.edit()`) — transform: `.with_columns/.assign/.astype/.drop/.rename/.apply` (or in place: `fr[col] = expr`, `del fr[col]`); row cuts: `.filter/.where/.report` (cutflow); reduce: `.sum/.mean/.min/.max/.count`; materialize: `.array/.dict/.recarray/.frame`; stream: `.iter_batches/.write(path)`. All transforms take `inplace=`.
...
- `saveas(source, dest, *, format=None)`, `to_root(source, path, ...)` — export (also `reader.to(name)`, `reader.arrow()`, `reader.to_*`).
