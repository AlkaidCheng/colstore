...
- **`ColStoreFrame`** is the edit interface. `reader.edit()` returns one; update,
  ...
  then either materialize in memory (`compute(name)` for one column, `dict()` /
  `recarray()` for all) or `.write(path)` to stream the result to a new file. It
  never modifies the source store.
