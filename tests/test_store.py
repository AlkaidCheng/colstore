...
def test_max_workers_property_uses_override_when_set(tmp_path, small_frame):
    ...
    with colstore.store(small_frame, path, show_progress=False, max_workers=3) as store:
...
def test_backend_property_reflects_constructor_choice(tmp_path, small_frame):
    ...
    with colstore.store(small_frame, path, show_progress=False, backend="numpy") as store:
