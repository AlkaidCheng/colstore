...
hardware (NERSC Perlmutter CPU nodes, dual AMD EPYC 7763) before merge.
...
**Key finding.** On quiet EPYC 7763 (Zen 3), software prefetch is a
...
(bracketed by two load-only proxies that agree tightly): the EPYC's
...
* **SIMD gather** — the bottleneck is outstanding-miss capacity, not
  instruction throughput; `vpgather` is microcoded on Zen 3.
